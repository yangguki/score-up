...
일을 시작하기 전에 [canon.md](canon.md)와 [docs-map.md](docs-map.md)를 읽는다. 화면·룰을 바꿀 때는 해당 원문 절까지 연다. 구현 진도는 작업계획 **§8**이 주인이다. 여기서 Phase 번호를 만들지 않는다.

## 출처 (원문)

캐논은 요약이다. 충돌하면 **원문이 이긴다.** 원문끼리 어긋나면 캐논·작업계획 §8 진도를 기준으로 맞추고 불일치를 문서에 남긴다.

| 주제 | 원문 |
| --- | --- |
| 정의·페르소나·모듈·로드맵 | `docs/SCORE-UP-기획서.md` v0.3.3 |
| 버튼·상태·종료·제재 | `docs/SCORE-UP-종목-룰-명세서.md` v0.2 |
| IA·화면·카피·MVP 체크 | `docs/SCORE-UP-화면기획.md` v0.4.13 |
| 구현 Phase·지금 안 하는 것·**진행 보드** | `docs/SCORE-UP-작업계획.md` v0.3 §8 |
| 홈 와이어·상태·카피·H8/H9 비교 | `docs/SCORE-UP-홈-상세.md` v0.3.5 |
| 대회 만들기 와이어 | `docs/SCORE-UP-대회-만들기-상세.md` v0.1 |
| 참가 팀/선수 와이어 | `docs/SCORE-UP-참가-상세.md` v0.1 |
| 모임·회차·참석 투표 | `docs/SCORE-UP-모임-기획.md` v0.8.3 |
| 모임 화면 와이어 | `docs/SCORE-UP-모임-상세.md` v0.6 |
| 급수 시즌·도전·결과 넣기 | `docs/SCORE-UP-급수-기획.md` v0.2 |
| 농구 보드 검수 항목 | `docs/SCORE-UP-스코어보드-레퍼런스-체크리스트.md` |
| 제품 방향 A/B/C (닫힘) | `docs/SCORE-UP-제품방향-비교.md` |
...
- 홈 **H1 제품 기본**. H8 Lift · H9 Play는 비교만. 채택 전까지 잠그지 않는다

## 작업 순서

1. 요청을 한 문장으로 재진술한다. 구현 요청이면 Manager에게 돌려보낸다.
2. 캐논에서 잠긴 결정·현재 Phase·명시적 제외를 확인한다. 진도는 작업계획 §8.
3. 해당 원문 절을 연다. 없으면 “문서 공백”으로 표시하고 초안만 쓴다.
4. 산출: 결정 표, 화면/흐름 절, 카피, 오픈 이슈. 코드를 쓰지 않는다.
5. 결정이 바뀌면 영향 받는 `docs/`와 이 폴더의 캐논을 같은 턴에 맞춘다. Manager §8이 필요하면 알린다.
...
| MatchEvent, canEndPeriod, 리듀서 | Rule Engine (농구 Phase 5 적용됨. 타 종목은 mock) |
| S3·공유 URL | Infra |
| 스택, 패키지, 새 종목 agent 생성, 일정, 진행 보드 | Manager |
| 배구/탁구 화면·프리셋 | volleyball / table-tennis |
| 배드민턴/스쿼시 화면·프리셋 | badminton / squash |
| 축구/풋살/야구 화면·프리셋 | soccer / futsal / baseball |
...
- 종목에 없는 UI를 명세에 넣기 (농구에 서브권 없음. 홈에 테니스·족구·기타 없음)
- 기획서 로드맵 Phase와 작업계획 Phase를 같은 번호로 섞어 말하기
- 원문을 안 고치고 채팅에서만 제품 결정하기
- H8/H9를 제품 기본 홈으로 잠그기
