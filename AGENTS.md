...
2. 종목 작업이면 해당 종목 skill을 읽고 Task로 넘긴다. 농구가 아니면 새 종목 agent가 있는지 먼저 확인한다.
3. 화면만 필요한 이번 단계는 **Frontend UX가 껍데기를 만들고, Basketball이 농구 화면의 버튼·상태·카피를 검수**한다.
4. 백엔드·실룰 엔진·다른 종목은 작업계획 Phase 5 이전에는 구현하지 않는다.
5. 필요한 agent가 없으면 Manager가 `.cursor/skills/`에 템플릿으로 만든 뒤 그 agent에게 일을 준다.
