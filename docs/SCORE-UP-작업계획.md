...
| 작성일 | 2026-08-20 |
| 수정일 | 2026-09-14 |
| 상태 | Phase 8 mock 닫힘. **다음 열린 일=배드민턴 MVP** (보드 UX·자체대회·계정 최소). H8/H9 비교 **동결**. 진도는 **§8·§9** |
| 목적 | Manager + 종목 agent 운영, 앱/웹 기술 조합, 구현 진도를 한곳에서 관리한다 |

기준: 기획서 v0.3.3, 종목 룰 명세서 v0.2, 화면 기획 v0.4.15, 모임 기획 v0.8.3, 홈 상세 v0.3.5
...
- [x] 모임 만들기 종목 **배드민턴** (당시 배구/탁구 잠금. 이후 §8에서 배구·풋살·탁구 열림)
...
- [x] 대회 만들기 1~4단계 — 홈 8종목 선택. 모임은 농구·배구·풋살·탁구·배드민턴
...
11. ~~모임 배구·풋살·탁구 mock~~ — 공통 층 재사용. 배구 6v6/4v4, 풋살 5v5/4v4, 탁구 한 판. 새 Phase 아님
...
구현 **Phase 8 닫힘**. 제품 홈은 **H1 Arena + CTA A 잠금**. 보드는 **가로만·농구 셸**. 모임 mock은 **농구·배구·풋살·탁구·배드민턴**. 종목 타일은 친선(피커 숨김). 지금 열린 일은 **홈 시안 비교**(H8 Lift · H9 Play). 새 구현 Phase가 아니다.
...
| 대회·친선 마법사 | 하단 `다음` 고정. 친선은 종목→룰→홈→어웨이(→농구 선수) 한 화면 한 설정 | Frontend UX |
| 토너먼트 대진표 | 가로·세로 트리 전환. 세로는 아래→위. 기본은 기기 방향. 보드는 가로만 | Frontend UX |
...
| 모임 | 농구 5v5·4v4 + 배구 6v6·4v4 + 풋살 5v5·4v4 + 탁구·배드민턴 한 판 | Frontend UX |
...
| 종목별 클럽 운영 확장 | **배구·풋살·탁구 mock 닫힘.** 스쿼시·축구·야구 모임은 아직. 회비·채팅 없음 |

### 8.5 이번 닫힌 일 (2026-08-28)

모임 공통 운영(멤버·초대승인·회차·투표·게스트·매칭 제안·전적·급수)은 이미 있었고, 종목 화이트리스트만 열었다. 새 구현 Phase 아님.

| 종목 | 회차 | 보드 |
| --- | --- | --- |
| 배구 | 12명이면 6v6, 8~11명이면 4v4(운영자 확정) | 기존 세트 보드. 출전 명단 스킵 |
| 풋살 | 농구와 같은 5v5 / 4v4 | 기존 전후반 보드. 출전 명단 스킵 |
| 탁구 | 배드민턴과 같은 단식·복식 한 판 | 기존 랠리 보드 |

`/club/new`는 농구·배구·풋살·탁구·배드민턴. 시드: 화요일 배구(참석 12), 목요일 풋살(참석 8), 금요일 탁구(참석 4). persist v7이 빠진 시드 모임을 합친다.

넣지 않음: 회비·채팅·GPS, 스쿼시·축구·야구 모임, 배드민턴 2차(멀티코트·복식 4명 결과).

---

## 9. 배드민턴 MVP (2026-09-14 잠금)

제품 Primary 가설을 **배드민턴 주간 픽업 + 월 자체대회**로 재잠금. 상세는 아래 문서.

| 문서 | 내용 |
| --- | --- |
| `docs/SCORE-UP-배드민턴-MVP-범위-확정서.md` | MVP 넣는 것/빼는 것 |
| `docs/SCORE-UP-배드민턴-보드-UX.md` | 터치 +1 · 21/25/커스텀 · 판수 커스텀 · 자체대회 기본 1판 |
| `docs/SCORE-UP-배드민턴-자체대회-참가신청.md` | 모임→대회 · 멤버 신청 |
| `docs/SCORE-UP-배드민턴-외부참가자-OCR.md` | 수동 명단 → OCR |
| `docs/SCORE-UP-계정-최소스펙.md` | 투표·신청용 계정 최소 |
| `docs/SCORE-UP-배드민턴-화면와이어-골격.md` | 화면 골격 |

### 9.1 다음 구현 순서 (P0)

1. 배드민턴 보드: 큰 터치존 +1, 목표점수 21/25/커스텀, 판수(1판·3판2선·5판3선·선승 커스텀)
2. 자체대회 기본 규칙 = **1판** (대회·경기 override 가능)
3. 모임→자체대회 `clubId` + 멤버 참가신청 (계정 최소 병행)
4. 외부 참가자 수동·붙여넣기
5. H8/H9 · 타 종목 엔진 · 배드민턴 모임 2차(회비·멀티코트) **동결**

### 9.2 홈 CTA (임시)

파일럿 기간 Primary 후보는 **이번 회차 / 내 모임 / 자체대회**. 대회 만들기는 Secondary·대회 탭. 확정 카피는 홈 상세 v0.4에서 반영.
