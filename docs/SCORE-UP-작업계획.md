...
| 작성일 | 2026-08-20 |
| 수정일 | 2026-08-27 |
| 상태 | Phase 8 mock 닫힘. 홈 **A 잠금**. 모임은 농구 + 배드민턴 1차(한 판). 실시간 링크는 API 후 |
| 목적 | Manager + 종목 agent 운영, 앱/웹 기술 조합, 농구 1종목 mock 프론트까지의 순서를 고정한다 |

기준: 기획서 v0.3, 종목 룰 명세서 v0.2, 화면 기획 v0.4, 모임 기획 v0.7
...
홈 A/B/C는 **A 잠금**. 설정 시안 전환 없음. B·C는 HTML 아카이브만.
...
- [x] 종목 보드 슬롯 (`volleyball`·`table-tennis`·`badminton`·`squash`·`soccer`·`futsal`·`baseball` mock)
- [x] 경기 시작 전에는 시계 정지
...
다음: Phase 8 배드민턴 모임 1차 **닫힘**. 홈 **A 잠금**. 실시간 공유 링크·이미지는 API 후. 태블릿 실물 검수는 선택. 배드민턴 2차(회비·멀티코트·복식 4명 결과)는 열지 않음.
