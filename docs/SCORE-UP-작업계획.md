...
| 작성일 | 2026-08-20 |
| 수정일 | 2026-08-26 |
| 상태 | Phase 6 mock. 홈 8종목 대회·친선 보드. 모임은 농구만. 실시간 링크는 API 후 |
...
홈 A/B/C 제품 잠금은 **나중에**. 기본 홈은 A. 비교 전환은 설정에 유지.

- [x] 농구 회차 **자동 매칭 제안** (무작위 / 승률 스네이크). 운영자 수정 후 확정 — `proposeClubSplit`
- [ ] 4v4 회차 (인원 미달 시)
- [ ] 월 정기 패턴
- [ ] 기간형 급수 리그 · 결과 업로드 · 도전장 (기획 초안 후)
...
- [x] 대회 만들기 1~4단계 — 홈 8종목 선택. 모임은 농구만
...
다음: **Phase 7** 모임 자동 매칭(진행). 홈 A/B/C는 나중에 결정. 실시간 공유 링크·이미지는 API 후. 태블릿 실물 검수는 선택. 모임 1차 포맷은 농구 5v5.
